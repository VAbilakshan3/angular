...
export class ListCustomersComponent implements OnInit {
  ...
  deleteCustomer(){
    ...
    this.customerService.deleteCustomer(this.deletedCustomer._id)
                      .subscribe((message: Message) => {
                          console.log(message);
                          // remove a deletedCustomer from customers list on view
                          this.customers = this.customers.filter(customer => {
                            return customer._id != this.deletedCustomer._id;
                          })
    ...
    this.customerService.updateCustomer(this.showCustomer)
                      .subscribe((message: Message) => {
                        ...
                        this.customers.map(x => {
                          if(x._id == this.showCustomer._id){
                            x = this.showCustomer;
                          }
                        });
                        ...
                        let msg: string = "Update Successfully! -> New Customer's properties: <br>"
                                          + "<ul>"
                                            + "<li>" + "id: " + this.showCustomer._id + "</li>"
